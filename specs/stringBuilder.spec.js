(function() {
	...
	describe("Each", function() {
	    ...
	    it("should send three parameters value, index and the given args array itself",
	       function() {
       		...
       		sb.each(arr, function(value, index, theGuy) {
    ...
    describe("Suspend", function() {
	    it("should return context", function() {
			sb = new StringBuilder();
			expect(sb.suspend()).toEqual(sb);
	    });

	    it("should suspend or pause the applied effects (prefix(), suffix() " +
	       "and wrap()) and it’s influence will finish with the call to end()" +
	       "method by restoring the previously added effects", function() {
			...
			sb.suffix('')
				...
				.each(sections, function(section, index){
					this.cat('<h1>', section, '</h1>', function() {
						this.suspend()
							.wrap('<p>', '</p>') 
							.cat('first paragraph') 
			...
			expect(sb.string()).toEqual("<body><section>" +
				"<h1>section 1</h1><p>first paragraph</p><p>second paragraph</p></section><section>" +
				"<h1>section 2</h1><p>first paragraph</p><p>second paragraph</p></section><section>" +
				"<h1>section 3</h1><p>first paragraph</p><p>second paragraph</p></section></body>");
		});
	});

	describe("When", function() {
		it("should evaluate the expression and call the cat﴾﴿ method with " +
		   "the thenArgs or otherwiseArgs depending on the result of " +
		   "evaluation", function() {
		   	sb = new StringBuilder();
			...
			sb.suffix(' ')
				...
				.each(people, function(person) {
					this.when(person.sex == 'm', function() {
							return person.name + ' is male'; 
						},
						[person.name, ' is female' ]);
			});

			expect(sb.string()).toEqual("<p>pedro is male</p>" +
